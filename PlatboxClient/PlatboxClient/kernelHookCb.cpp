...
void hook_kernel_callback(HANDLE h, PVOID target_address) {
	PATCH_CALLBACK p = { 0 };
	p.contentSize = 0;
	...
	NTSTATUS status = DeviceIoControl(h, IOCTL_PATCH_CALLBACK,
...
void hook_kernel_callback_with_shellcode(HANDLE h, PVOID target_address, char *shellcode) {
	PATCH_CALLBACK p = { 0 };
...
void restore_kernel_callback(HANDLE h, PVOID target_address) {
	RESTORE_CALLBACK p = { 0 };
	p.targetAddress = target_address;

	DWORD bytesReturned = 0;
	NTSTATUS status = DeviceIoControl(h, IOCTL_RESTORE_CALLBACK,
...
void remove_all_kernel_callbacks(HANDLE h) {
	DWORD bytesReturned = 0;
	NTSTATUS status = DeviceIoControl(h, IOCTL_REMOVE_ALL_CALLBACKS_HOOKS,
		NULL, NULL,
		NULL, NULL,
		&bytesReturned, NULL);
	if (NT_SUCCESS(status)) {
...
void parse_kernel_hook_operation(HANDLE h, int argc, char **argv) {
	if (!strcmp(argv[1], "patch")) {
		...
		if (argc == 3) {
			...
			hook_kernel_callback(h, (PVOID)address);
		}
		if (argc == 4) {
			// kernelhook patch ffff89868ba1ce30 909090c3 
			UINT64 address = strtoull(argv[2], NULL, 16);
			hook_kernel_callback_with_shellcode(h, (PVOID)address, argv[3]);
		}
	}
	if (!strcmp(argv[1], "restore")) {
		if (argc == 3) {
			UINT64 address = strtoull(argv[2], NULL, 16);
			restore_kernel_callback(h, (PVOID)address);
		}
	}
	if (!strcmp(argv[1], "remove")) {		
		remove_all_kernel_callbacks(h);
	}
}
